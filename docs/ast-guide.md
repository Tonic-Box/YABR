...
- `try`/`catch`/`finally` - de-duplicating javac's inlined finally copies (the normal-path copy,
  each early-exit copy, and the catch-all handler) back into a single `finally` block
...
Invalid source fails loudly: an assignment to a name that is neither a declared local, a
parameter, nor a resolvable field throws `LoweringException` rather than producing bytecode
with the store silently dropped.
...
When a variable is only used inside one nested block (a try body or an if arm, not a loop body -
a loop would re-run the initializer), the declaration is moved inside:
...
`ClassDecompiler` runs a fixed AST transform pipeline on every recovered body (methods and
constructors alike): control-flow simplification, pattern-`instanceof` reconstruction,
single-use inlining, dead-store/dead-variable elimination, varargs and array-literal
reconstruction, declaration hoisting/sinking, loop-counter folding into `for`-inits,
switch reconstruction, and scope-escape repair. `ControlFlowSimplifier` runs twice - the
eliminators expose shapes (e.g. an emptied then-branch) the first pass cannot see.
