...
public class VarargsReconstructor implements ASTTransform
{
     ...
     * @param classFile the class being decompiled, used to resolve callees
     *        declared in it before falling back to the JDK
     */
    ...
    private boolean process(List<Statement> stmts, Map<String, Integer> uses)
    {
        for (int i = 0; i < stmts.size(); i++)
        {
            ...
            if (build != null && uses.getOrDefault(build.varName, 0) == build.elements.size() + build.extraRefs + 1)
            {
    ...
    private ArrayBuild matchArrayBuild(List<Statement> stmts, int index, Map<String, Integer> uses)
    {
        ...
        while (matched < size)
        {
            ...
            if (assign.getOperator() != BinaryOperator.ASSIGN || !(assign.getLeft() instanceof ArrayAccessExpr))
            {
                return null;
            }
