...
public class PatternSwitchReconstructor implements ASTTransform
{
    ...
    private WhileStmt asDispatchLoop(Statement s)
    {
        ...
        if (s instanceof LabeledStmt && ((LabeledStmt) s).getStatement() instanceof WhileStmt)
        {
    ...
    private boolean isRestartBody(Statement branch, String restartVar)
    {
        ...
        if (stmts.size() != 2 || !(stmts.get(1) instanceof ContinueStmt) || ((ContinueStmt) stmts.get(1)).hasLabel())
        {
            return false;
        }
        if (!(stmts.get(0) instanceof ExprStmt) || !(((ExprStmt) stmts.get(0)).getExpression() instanceof BinaryExpr))
        {
            return false;
        }
