        ...
        switch (op)
        {
            case MUL:
                if (rightConst != null && isPowerOfTwo(rightConst))
                {
                    ...
                    return new BinaryOpInstruction(result, BinaryOp.SHL, left, IntConstant.of(shift));
                }
                if (leftConst != null && isPowerOfTwo(leftConst))
                {
                    int shift = Integer.numberOfTrailingZeros(leftConst);
                    return new BinaryOpInstruction(result, BinaryOp.SHL, right, IntConstant.of(shift));
                }
                break;
            ...
            case DIV:
                ...
                {
                    int shift = Integer.numberOfTrailingZeros(rightConst);
                    return new BinaryOpInstruction(result, BinaryOp.SHR, left, IntConstant.of(shift));
                }
                break;
            ...
            case REM:
                if (rightConst != null && rightConst > 0 && isPowerOfTwo(rightConst))
                {
                    ...
                    return new BinaryOpInstruction(result, BinaryOp.AND, left, IntConstant.of(mask));
                }
                break;
