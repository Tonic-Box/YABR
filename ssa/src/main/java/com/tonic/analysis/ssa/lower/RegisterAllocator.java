...
public class RegisterAllocator
{
    ...
    /**
     * Creates an allocator; nothing is assigned until {@link #allocate()} runs.
     *
     * @param method the method whose values need slots
     * @param liveness the liveness result driving interval construction
     */
    ...
    /**
     * @return the reserved slot count
     */
    ...
    /**
     * Runs the linear scan, pinning parameters to the leading slots and letting later values
     * reuse a slot only when no named source variable already owns it.
     */
    ...
    /**
     * @return the affinity pinned values
     */
    ...
    private SSAValue copySource(CopyInfo copyInfo)
    {
        for (IRInstruction instr : copyInfo.block().getInstructions())
        {
            if (instr instanceof CopyInstruction)
            {
                ...
                if (copy.getResult().equals(copyInfo.copyValue()) && copy.getSource() instanceof SSAValue)
                {
    ...
    /**
     * Looks up a value's slot, allocating a fresh one and growing max locals if the main pass
     * missed it.
     *
