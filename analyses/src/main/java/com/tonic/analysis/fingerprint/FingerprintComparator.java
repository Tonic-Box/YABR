...
public class FingerprintComparator
{
     ...
     * @return a match with the weighted score, confidence, and per-level scores
     */
    public FingerprintMatch compare(MethodFingerprint a, MethodFingerprint b)
    {
        ...
        if (a.hasLevel(FingerprintLevel.ULTRA_STABLE) && b.hasLevel(FingerprintLevel.ULTRA_STABLE))
        {
        ...
        if (a.hasLevel(FingerprintLevel.STABLE) && b.hasLevel(FingerprintLevel.STABLE))
        {
        ...
        if (a.hasLevel(FingerprintLevel.DETAILED) && b.hasLevel(FingerprintLevel.DETAILED))
        {
        ...
        if (totalWeight > 0)
        {
            weightedScore = (scores[0] * weights[0] + scores[1] * weights[1] + scores[2] * weights[2]) / totalWeight;
        }

        double confidence = computeConfidence(scores, weights, levelsMatched);

        return new FingerprintMatch(a.getMethodId(), b.getMethodId(), weightedScore, confidence, scores);
    }
     ...
     * @return true if the match clears both thresholds
     */
     ...
     * @param match the match to test
     * @return true if the match clears both fixed thresholds
     */
