...
public class CallGraphBuilder
{
     ...
     * @param classPool the pool of classes to scan
    ...
    public static CallGraph build(ClassPool classPool)
    {
        ...
        for (ClassFile cf : classes)
        {
            ...
            {
                MethodReference ref = new MethodReference(cf.getClassName(), method.getName(), method.getDesc());
        ...
        for (ClassFile cf : classes)
        {
            for (MethodEntry method : cf.getMethods())
            {
                ...
                MethodReference callerRef = new MethodReference(cf.getClassName(), method.getName(), method.getDesc());
     ...
     */
    private static void scanMethodForCalls(CallGraph graph, MethodReference callerRef, MethodEntry method, ClassFile cf)
    {
        try
        {
     ...
     */
    private static void processInvokeInstruction(CallGraph graph, CallGraphNode callerNode, MethodReference callerRef, InvokeInstruction invoke, ConstPool constPool, BootstrapMethodsAttribute bsmAttr)
    {
    ...
    private static void processDynamicInvoke(CallGraph graph, CallGraphNode callerNode, MethodReference callerRef, InvokeInstruction invoke, String targetName, String targetDesc, ConstPool constPool, BootstrapMethodsAttribute bsmAttr)
    {
        ...
        {
            MethodReference bsmRef = new MethodReference(bsm.getOwner(), bsm.getName(), bsm.getDescriptor());
    ...
    private static void processBootstrapArgument(CallGraph graph, CallGraphNode callerNode, MethodReference callerRef, Constant arg, ConstPool constPool, BootstrapMethodsAttribute bsmAttr)
    {
        if (arg instanceof MethodHandleConstant)
        {
            ...
            {
                MethodReference target = new MethodReference(mh.getOwner(), mh.getName(), mh.getDescriptor());
                addCallEdge(graph, callerNode, callerRef, target, InvokeType.DYNAMIC);
            }
        }
     ...
     */
    private static void processConstantInstruction(CallGraph graph, CallGraphNode callerNode, MethodReference callerRef, ConstantInstruction constInstr, ConstPool constPool, BootstrapMethodsAttribute bsmAttr)
    {
     ...
     */
    private static void addCallEdge(CallGraph graph, CallGraphNode callerNode, MethodReference callerRef, MethodReference targetRef, InvokeType invokeType)
    {
